...
type ThaiNumeralConverter () =
    interface INumeralConverter with
        member this.ToNumeral number = Numeral.toThai number
        member this.TryParse (s, result) =
            Helper.tryParse Numeral.tryParseThai (s, &result)
...
type Numeral private () =
    ...
    static member val Thai  =  ThaiNumeralConverter () :> INumeralConverter